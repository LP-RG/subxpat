...
def auto_run(experiment_num):
    command="python3 main.py mul_i4_o4 --metric wre --min-labeling --extraction-mode 0 --subxpat --encoding z3bvec --max-error 25 --imax 2 --omax 2 --template nonshared --max-lpp 4 --max-ppo 4"
    ...
    with open("description.txt", "w") as file:
        ...
        # for i in codes:
        #     file.write(i+"\n")
    subprocess.run("mkdir output/mul_i4_o4_exp"+str(experiment_num),shell=True)
    subprocess.run("mv description.txt output/gv output/ver output/z3 output/report output/mul_i4_o4_exp"+str(experiment_num),shell=True)
