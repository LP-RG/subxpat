...
from sxpat.converting.utils import get_nodes_type, set_bool_constants, crystallise as crystallize
...
class QbfSolver(Solver):
    ...
    def _solve(cls,
        ...
        result = subprocess.run(['cqesto', script_path], capture_output=True, text=True)
    ...
    def solve(cls, graphs: _Graphs, specifications: Specifications) -> Tuple[str, Optional[Mapping[str, Union[bool, int]]]]:
        ...
        graphs = [
            *graphs,
            CGraph(
                chain.from_iterable(
                    (PlaceHolder(node.name), Constraint.of(node))
        ...
        return cls._solve(graphs, specifications, list(found_inputs_names.pop()))
