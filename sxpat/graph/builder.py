...
class GraphBuilder:
    ...
    class NoNodesPresentError(Exception): pass
    ...
    def last_name(self) -> str:
        ...
        if self.__last_name is None: raise self.NoNodesPresentError()
    ...
    def mark_inputs(
        ...
        # elaborate
        self.__extras['inputs_names'] = self.__extras.get('inputs_names', ()) + tuple(inputs_names)
    ...
    def mark_input(
        ...
        """
        Marks the wanted node as input (or the last node added if no name is given).

        :param input_name: The name of the node to be marked as input.
        :return self:      The builder object.
        """

        # guards
        if self.__is_built: raise self.AlreadyBuiltError()
        if input_name not in self.__partial_nodes: raise self.InvalidNodeError(input_name)

        # elaborate
        self.__extras['inputs_names'] = self.__extras.get('inputs_names', ()) + (input_name, )
    ...
    def mark_outputs(
        ...
        # elaborate
        self.__extras['outputs_names'] = self.__extras.get('outputs_names', ()) + tuple(outputs_names)
    ...
    def mark_output(
        ...
        """
        Marks the wanted node as output (or the last node added if no name is given).

        :param output_name: The name of the node to be marked as output.
        :return self:       The builder object.
        """

        # guards
        if self.__is_built: raise self.AlreadyBuiltError()
        if output_name not in self.__partial_nodes: raise self.InvalidNodeError(output_name)

        # elaborate
        self.__extras['outputs_names'] = self.__extras.get('outputs_names', ()) + (output_name, )
    ...
    def mark_parameters(
        ...
        # elaborate
        self.__extras['parameters_names'] = self.__extras.get('parameters_names', ()) + tuple(parameters_names)
    ...
    def mark_parameter(
        ...
        """
        Marks the wanted node as parameter (or the last node added if no name is given).

        :param parameter_name: The name of the node to be marked as parameter.
        :return self:          The builder object.
        """

        # guards
        if self.__is_built: raise self.AlreadyBuiltError()
        if parameter_name not in self.__partial_nodes: raise self.InvalidNodeError(parameter_name)

        # elaborate
        self.__extras['parameters_names'] = self.__extras.get('parameters_names', ()) + (parameter_name, )
    ...
    class PipableCallable(Protocol):
        def __call__(self, builder: GraphBuilder, *args, **kwargs) -> Any: ...

    def update_with(
            self,
            func: PipableCallable, *args, **kwargs,
    ) -> Self:
        """
