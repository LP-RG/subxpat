...
from typing import Iterator, Optional, Tuple, List, Callable, Any, Union
...
class Template_SOP1(TemplateCreator):
    ...
    def generate_z3py_subinput_variables(self, prefix: str):
        input_variables = f'# Subgraph inputs variables declaration\n'
        for name in self.current_graph.subgraph_input_dict.values():
            name = f"{prefix}_sub_{name}"
            input_variables += self.declare_gate(name)
        input_variables += '\n'
    ...
    def generate_z3py_script_v2_phase1(self):
        ...
        z3_abs_function = self.z3_generate_z3_abs_function()  # parent
        input_variables_declaration = self.z3_generate_declare_input_variables()
        subinputs_1 = self.generate_z3py_subinput_variables("MARCO")
        subinputs_2 = self.generate_z3py_subinput_variables("TIRE")

        exact_integer_function_declaration = self.z3_generate_declare_integer_function("MARCO_" + F_EXACT)
        approximate_integer_function_declaration = self.z3_generate_declare_integer_function("TIRE_" + F_EXACT)
        utility_variables = self.z3_generate_utility_variables((F_EXACT, F_EXACT), ("MARCO", "TIRE"))  # TODO HERE HERE
        # implicit_parameters_declaration = self.z3_generate_declare_implicit_parameters_subxpat()
        # TODO: MARCO
        circuit_1 = "".join([
            self.z3_generate_exact_circuit_wires_declaration("MARCO"),  # exact_circuit_wires_declaration
            self.z3_generate_exact_circuit_outputs_declaration("MARCO"),  # exact_circuit_outputs_declaration
            self.z3_generate_exact_circuit_constraints("curcuit_1", "MARCO"),  # exact_circuit_constraints
        ])
        circuit_2 = "".join([
            self.z3_generate_exact_circuit_wires_declaration("TIRE"),  # exact_circuit_wires_declaration
            self.z3_generate_exact_circuit_outputs_declaration("TIRE"),  # exact_circuit_outputs_declaration
            self.z3_generate_exact_circuit_constraints("curcuit_2", "TIRE"),  # exact_circuit_constraints
        ])
        # TODO: MARCO
        # approximate_circuit_wires_declaration = self.z3_generate_approximate_circuit_wires_declaration_subxpat()
        # approximate_circuit_outputs_declaration = self.z3_generate_approximate_circuit_outputs_declaration()
        ...
        z3pyscript = "".join([
            ...
            subinputs_1,
            subinputs_2,
            exact_integer_function_declaration,
            approximate_integer_function_declaration,
            utility_variables,
    ...
    def z3_generate_utility_variables(self, function_names: Tuple[str] = (F_EXACT, F_APPROXIMATE), prefixes: Tuple[str] = None):
        prefixes = ("", "") if prefixes is None else (f"{prefixes[0]}_", f"{prefixes[1]}_")
        utility_variables = ''
        utility_variables += f'# utility variables'
        utility_variables += f"\n" \
                             f"difference = z3_abs({prefixes[0]}{function_names[0]}({', '.join(self.exact_graph.input_dict.values())}) - " \
                             f"{prefixes[1]}{function_names[1]}({', '.join(self.exact_graph.input_dict.values())})" \
                             f")\n" \
                             f"error = {Z3INT}('error')\n"
    ...
    def z3_generate_exact_circuit_wires_declaration(self, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        # NOTE: Updated by Marco
        # TODO: update when merging #2
        return "\n".join([
            f"# wires functions declaration for exact circuit",
            *(
                (
                    f"{prefix}{EXACT_WIRES_PREFIX}{self.exact_graph.num_inputs + g_idx} = "
                    f"{FUNCTION}('{prefix}{EXACT_WIRES_PREFIX}{self.exact_graph.num_inputs + g_idx}', "
    ...
    def z3_generate_v2_phase1_circuit(self, id: int):
        # NOTE: Created by Marco
        # TODO: update when merged `format_lines`
        # return "\n".join([
        #     f"# wires functions declaration for approximate circuit",
        #     (
        #         (
        #             f"{APPROXIMATE_WIRE_PREFIX}{self.current_graph.num_inputs + g_idx} = "
        #             f"{FUNCTION}('{APPROXIMATE_WIRE_PREFIX}{self.current_graph.subgraph_num_inputs + g_idx}', "
        #             f"{', '.join(repeat(BOOLSORT, self.current_graph.subgraph_num_inputs))}, {BOOLSORT})"
        #         )
        #         for g_idx in range(self.current_graph.num_gates)
        #     )
        # ])
        for g_idx in range(self.current_graph.num_gates):
            g_label = self.current_graph.gate_dict[g_idx]
            # print(g_label, self.current_graph.is_subgraph_member(g_label))
            # print(
            #     f"{APPROXIMATE_WIRE_PREFIX}{self.current_graph.num_inputs + g_idx} = "
            #     f"{FUNCTION}('{APPROXIMATE_WIRE_PREFIX}{self.current_graph.subgraph_num_inputs + g_idx}', "
            #     f"{', '.join(repeat(BOOLSORT, self.current_graph.subgraph_num_inputs))}, {BOOLSORT})"
    ...
    def z3_generate_exact_circuit_outputs_declaration(self, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        ...
        for output_idx in range(self.current_graph.num_outputs):
            exact_circuit_output_declaration += f"{prefix}{EXACT_OUTPUT_PREFIX}{OUT}{output_idx} = {FUNCTION} ('{prefix}{EXACT_OUTPUT_PREFIX}{OUT}{output_idx}', " \
                                                f"{', '.join(repeat(BOOLSORT, self.current_graph.num_inputs + 1))}" \
                                                f")\n"
        exact_circuit_output_declaration += '\n'
        return exact_circuit_output_declaration

    def z3_generate_approximate_circuit_outputs_declaration(self, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        ...
        for output_idx in range(self.current_graph.num_outputs):
            approximate_circuit_output_declaration += f"{prefix}{APPROXIMATE_OUTPUT_PREFIX}{OUT}{output_idx} = {FUNCTION} ('{prefix}{APPROXIMATE_OUTPUT_PREFIX}{OUT}{output_idx}', " \
                                                      f"{', '.join(repeat(BOOLSORT, self.current_graph.num_inputs + 1))}" \
                                                      f")\n"
    ...
    def z3_generate_exact_circuit_wire_constraints(self, circuit_name: str = EXACT_CIRCUIT, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        exact_wire_constraints = ''
        exact_wire_constraints += f'# exact circuit constraints\n'
        exact_wire_constraints += f'{circuit_name} = And(\n'
        ...
        for g_idx in range(self.exact_graph.num_gates):
            ...
            if len(g_predecessors) == 1:
                if g_predecessors[0] in list(self.exact_graph.input_dict.values()):
                    ...
                    pred_1 = prefix + self.z3_express_node_as_wire_constraints(g_predecessors[0])
                exact_wire_constraints += f"{TAB}{prefix}{EXACT_WIRES_PREFIX}{self.exact_graph.num_inputs + g_idx}(" \
                ...
                if g_predecessors[1] in list(self.exact_graph.input_dict.values()):
                    ...
                    pred_2 = prefix + self.z3_express_node_as_wire_constraints(g_predecessors[1])

                exact_wire_constraints += f"{TO_Z3_GATE_DICT[g_function]}({pred_1}, {pred_2}),\n"
        return exact_wire_constraints

    def z3_generate_exact_circuit_output_constraints(self, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        ...
        for output_idx in self.exact_graph.output_dict.keys():
            ...
            if output_predecessors[0] in list(self.exact_graph.input_dict.values()):
                ...
                pred = prefix + self.z3_express_node_as_wire_constraints(output_predecessors[0])
            output = self.z3_express_node_as_wire_constraints(output_label)
            exact_output_constraints += f'{TAB}{prefix}{output} == {pred},\n'
        return exact_output_constraints

    def z3_generate_exact_circuit_integer_output_constraints(self, prefix: str = None):
        prefix = "" if prefix is None else f"{prefix}_"
        exact_integer_outputs = ''
        exact_integer_outputs += f'{TAB}# exact_integer_outputs\n'
        exact_integer_outputs += f"{TAB}{prefix}{F_EXACT}({','.join(self.current_graph.input_dict.values())}) == \n"

        for idx in range(self.current_graph.num_outputs):
            output_label = self.current_graph.output_dict[idx]
            if idx == self.current_graph.num_outputs - 1:
                exact_integer_outputs += f"{TAB}{2 ** idx} * {prefix}{self.z3_express_node_as_wire_constraints(output_label)},\n"
            else:
                exact_integer_outputs += f"{TAB}{2 ** idx} * {prefix}{self.z3_express_node_as_wire_constraints(output_label)} +\n"
    ...
    def z3_generate_exact_circuit_constraints(self, circuit_name: str = EXACT_CIRCUIT, prefix: str = None):
        wires = self.z3_generate_exact_circuit_wire_constraints(circuit_name, prefix)
        outputs = self.z3_generate_exact_circuit_output_constraints(prefix)
        integer_outputs = self.z3_generate_exact_circuit_integer_output_constraints(prefix)
        return "\n".join([
            wires,
            outputs,
            integer_outputs,
...
class Template_V2(Template_SOP1):
    def z3_generate_exact_circuit_constraints(self, circuit_name: str, prefix: str):
        # NOTE: ok

        return format_lines(
            [
                self.z3_generate_exact_circuit_wire_constraints(circuit_name, prefix),  # wires
                self.z3_generate_exact_circuit_output_constraints(prefix),              # outputs
                self.z3_generate_exact_circuit_integer_output_constraints(prefix),      # integer_outputs
                ")"
            ],
            extra_newlines=1
        )

    def prefixed_name(self, raw: str, prefix: str) -> str:
        # NOTE: ok

        if raw in self.exact_graph.input_dict.values():
            return raw
        return prefix + self.z3_express_node_as_wire_constraints(raw)

    def z3_generate_exact_circuit_wire_constraints(self, circuit_name: str, prefix: str):
        # NOTE: ok

        prefix = f"{prefix}_"
        def prefixed_name(raw): return self.prefixed_name(raw, prefix)
        ...
        for g_idx in range(self.exact_graph.num_gates):
            ...
            if len(g_predecessors) == 1:
                ...
                lines.append(
                    f"{TAB}{prefix}{EXACT_WIRES_PREFIX}{self.exact_graph.num_inputs + g_idx}"
    ...
    def z3_generate_exact_circuit_output_constraints(self, prefix: str):
        # NOTE: ok
    ...
    def z3_generate_exact_circuit_integer_output_constraints(self, function_name: str, prefix: str):
        # NOTE: ok

        prefix = f"{prefix}_"
        return format_lines(
            [
                f"# exact_integer_outputs",
                f"{prefix}{function_name}({','.join(self.current_graph.input_dict.values())}) == ",
                " + ".join(
                    f"{2 ** idx} * {prefix}{self.z3_express_node_as_wire_constraints(self.current_graph.output_dict[idx])}"
                    for idx in range(self.current_graph.num_outputs)
                ) + ","
            ],
            indent_amount=1,
    ...
    def z3_generate_exact_circuit_outputs_declaration(self, prefix: str):
        # NOTE: ok
        ...
        return format_lines(
            [
                ...
                *(
                    (
                        f"{prefix}{EXACT_OUTPUT_PREFIX}{OUT}{output_idx} = {FUNCTION} "
                        f"('{prefix}{EXACT_OUTPUT_PREFIX}{OUT}{output_idx}', {', '.join(repeat(BOOLSORT, self.current_graph.num_inputs + 1))})"
                    )
                    for output_idx in range(self.current_graph.num_outputs)
                )
            ],
    ...
    def z3_generate_exact_circuit_wires_declaration(self, prefix: str):
        # NOTE: ok
    ...
    def generate_z3py_subinput_variables(self, prefix: str):
        ...
        return format_lines(
            [
                f"# Subgraph inputs variables declaration",
                *(
                    self.declare_gate(f"{prefix}_sub_{name}").strip()
                    for name in self.current_graph.subgraph_input_dict.values()
                )
            ],
            extra_newlines=1
