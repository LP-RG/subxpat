...
class InputsReplace:
    ...
    def define(cls, current: IOGraph, specs_obj: Specifications, selected_nodes, replace_output=True, replace_with_constant=None):
        ...
        # if specs_obj.slash_inputs_relative or specs_obj.relative:
        #     new_nodes.extend([
        #         zero := IntConstant('Zero', value=0),
        #         one := IntConstant('One', value=1),
        #         abs_diff := AbsDiff('weight', operands=(cur_int.name, tem_int.name)),
        #         relative_constraint := LessEqualThan('relative_constraint', operands=(abs_diff.name, cur_int.name)),
        #         Constraint.of(relative_constraint),
        #     ])
