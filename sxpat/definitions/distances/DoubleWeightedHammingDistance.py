...
__all__ = ['DoubleWeightedHammingDistance']


class DoubleWeightedHammingDistance(DistanceSpecification):
    """
        ...
        the value is divided when switching from 1 to 0 and viceversa.
    """
    @override
    @classmethod
    ...
    def _minimum_distance(cls, graph_a,
        ...
        return min(min(int(graph_a[n].weight.split()[0]),int(graph_a[n].weight.split()[1])) for n in wanted_a)