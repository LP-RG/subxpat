        ...
        if self.__base_script is None:
            with self.__lock:
                if self.__base_script is None:
                    ...
                    strings.append(f'constf = IntVal(0)\n')
                    for i in range(num_outputs):
                        # strings.append(f'const{i} = BitVecVal({2**i}, {num_outputs})\n')
                        strings.append(f'const{i} = IntVal({2**i})\n')
                    strings.append('\n')
                    ...
                    if self._use_func:
                        ...
                        strings.append(f'error = Function("error", IntSort(), IntSort(), IntSort())\n')
                        strings.append('solver.add(error(r_out, l_out) == If(r_out >= l_out, r_out - l_out, l_out - r_out))\n')
                        if self._minimize:
                        ...
                        strings.append('error = If(r_out >= l_out, r_out - l_out, l_out - r_out)\n')
                        if self._minimize:
            ...
            with self.__lock:
                if self.__base_script is None:
                    ...
                    if self._use_func:
                        ...
                        strings.append('error = If(UGE(r_out, l_out), r_out - l_out, l_out - r_out)\n')
                        if self._minimize:
                            strings.append('solver.add(UGE(error, 0))\n')
                            strings.append('opt_objective = solver.minimize(error)\n\n')
                        else:
                            strings.append('opt_objective = solver.maximize(error)\n\n')
                        # run
