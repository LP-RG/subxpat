...
from typing import Callable, Collection, Dict, Iterable, Sequence, Tuple, Optional, Text
...
class TemplateManager:
    ...
    def factory(specs: Specifications,
        ...
        return {
            ...
            (TemplateType.V2, EncodingType.Z3_BITVECTOR): Subxpat_v2,
...
class Subxpat_v2(SOPManager):
    def __init__(self, exact_graph: AnnotatedGraph,
                 current_graph: AnnotatedGraph,
                 specs: Specifications,
                 encoding: Encoding,
                 ) -> None:
        super().__init__(exact_graph, current_graph,specs,encoding)

        self.executor = SubXPatV2Executor(
            None, None, self._specs.exact_benchmark,
            None, None,
            None, None,
            self._specs.et
        )
    ...
    def set_new_context(self, specs: Specifications, et):
        ...
        self.template_name = specs.template_name
        self.benchmark_name = specs.current_benchmark
    ...
    def set_graph_and_update_functions(self, annotated_graph: AnnotatedGraph):
        ...
        if self.sub_error_function == 1:
            subcircuit_distance_function = WeightedAbsoluteDifference(
                sub_graph.inputs,
                [annotated_graph.subgraph.nodes[n][sxpat_cfg.WEIGHT] for n in sub_graph.unaliased_outputs]
            )
    ...
    def import_json_model(self, this_path=None):
        ...
        for d in data:
            for key in d.keys():
                if key == sxpat_cfg.RESULT:
                    if d[key] == sxpat_cfg.SAT:
                        self.json_model.append(d[sxpat_cfg.MODEL])
                        self.json_status.append(sxpat_cfg.SAT)
                    elif d[key] == sxpat_cfg.UNSAT:
                        self.json_model.append(None)
                        self.json_status.append(sxpat_cfg.UNSAT)
                    else:
                        self.json_model.append(None)
                        self.json_status.append(sxpat_cfg.UNKNOWN)
