...
class AnnotatedGraph(Graph):
            ...
            if specs_obj.requires_subgraph_extraction:
                ...
                if mode == 1:
                    ...
                    if self.subgraph_candidates:
                        ...
                        pprint.info2(
                            f"Partition with omax={specs_obj.omax} and soft feasibility constraints on subgraph outputs, and limited inputs. Looking for largest partition")
                        self.subgraph = self.find_subgraph_feasible_soft_outputs_limited_inputs(
                            specs_obj)  # Critian's subgraph extraction
                    cnt_nodes = 0
                    for gate_idx in self.gate_dict:
                        if self.subgraph.nodes[self.gate_dict[gate_idx]][SUBGRAPH] == 1:
                    ...
                    pprint.info2(
                        f"Partition with omax={specs_obj.omax} and hard feasibility constraints on inputs and outputs. Looking for largest partition")
                    self.subgraph = self.find_subgraph_feasible_hard_limited_inputs(specs_obj)  # Critian's subgraph extraction
                    cnt_nodes = 0
                    for gate_idx in self.gate_dict:
                        if self.subgraph.nodes[self.gate_dict[gate_idx]][SUBGRAPH] == 1:
                            cnt_nodes += 1
                    ...
                    pprint.info2(
                        f"Partition with omax={specs_obj.omax} and hard feasibility constraints on inputs and outputs. Looking for largest partition")
                    self.subgraph = self.find_subgraph_feasible_hard_limited_inputs_refined(
                        specs_obj)  # Critian's subgraph extraction
                    cnt_nodes = 0
                    for gate_idx in self.gate_dict:
                        if self.subgraph.nodes[self.gate_dict[gate_idx]][SUBGRAPH] == 1:
                    ...
                    pprint.info2(
                        f"Partition with omax={specs_obj.omax} and hard constraints, imax, omax, assumptions, and BitVec. Looking for largest partition")
                    self.subgraph = self.find_subgraph_feasible_hard_limited_inputs_refined_bitvec(
                        specs_obj)  # Critian's subgraph extraction
                    cnt_nodes = 0
                    for gate_idx in self.gate_dict:
                        if self.subgraph.nodes[self.gate_dict[gate_idx]][SUBGRAPH] == 1:
                    ...
                    pprint.info2(
                        f"Partition with omax={specs_obj.omax} and hard constraints, imax, omax, assumptions, and BitVec, DataType. Looking for largest partition")
                    self.subgraph = self.find_subgraph_feasible_hard_limited_inputs_datatype_bitvec_assumptions(
                        specs_obj)  # Critian's subgraph extraction
                    cnt_nodes = 0
                    for gate_idx in self.gate_dict:
                        if self.subgraph.nodes[self.gate_dict[gate_idx]][SUBGRAPH] == 1:
    ...
    def find_subgraph_feasible_hard_limited_inputs_refined(self, specs_obj: TemplateSpecs):
        ...
        skipped_nodes_constraints = [node_literal == False for node_literal in skipped_nodes]
        # print(f'{skipped_nodes = }')
        # print(f'{skipped_nodes_constraints = }')
        ...
        for i in range(len(node_partition) - 1):
            for j in range(i + 1, len(node_partition)):
                ...
                try:
                    ...
                    # print("No path", u, v)
                    pass
        print(f'Convex_check = {time.time() - convex_check}')
    ...
    def find_subgraph_feasible_hard_limited_inputs_datatype_bitvec_assumptions(self, specs_obj: TemplateSpecs):
        ...
        for in_idx in self.input_dict:
            ...
            opt.add(Node.weight(node) == BitVecVal(weight, 32))
            # opt.add(Node.in_subgraph(node) == BoolVal(False))
            skipped_nodes.append(Node.in_subgraph(node) == BoolVal(False))
            nodes[node_label] = node
