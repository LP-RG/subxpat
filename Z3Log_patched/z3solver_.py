...
from Z3Log import z3solver as _Z3solver


class Z3solver(_Z3solver):
    # def patched_new()
    def __init__(self, circuit_path: str,
                 output_graphviz_path: str, temporary_path: str,
                 # this is actually used
                 approximate_circuit_path: str = None,
        ...
        # TODO: make sure graph works with path
        self.__circuit_name = circuit_name = get_pure_name(circuit_path)
        self.__graph_in_path = f'{output_graphviz_path}/{circuit_name}.gv'
        self.__graph = Graph(circuit_path, True)

        #
        os.makedirs(f'{temporary_path}/{circuit_name}_z3py', exist_ok=True)
        self.__z3_log_path = f'{temporary_path}/{circuit_name}_z3py/{circuit_name}_z3py.log'

        if approximate_circuit_path:
            # TODO: make sure graph works with path
            self.__approximate_circuit_name = get_pure_name(approximate_circuit_path)
            self.__approximate_graph = Graph(approximate_circuit_path, True)
    ...
    def import_labels(self, constant_value: bool = False) -> Dict:
        ...
        for _dir in all_dirs:
            if re.search(f'{self.approximate_benchmark}_labeling', _dir) and os.path.isdir(f'{folder}/{_dir}') and re.search(f'{constant_value}', _dir):
                relevant_dir = f'{folder}/{_dir}'
